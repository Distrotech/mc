                      ...
                      uintmax_t * ret_total, gboolean compute_symlinks)
{
    ...
    for (i = 0; i < panel->dir.len; i++)
    {
        ...
        if (S_ISDIR (s->st_mode))
        {
            ...
            status = compute_dir_size (p, sm, ret_count, ret_total, compute_symlinks);
            vfs_path_free (p);
                           ...
                           filegui_dialog_type_t dialog_type)
{
    ...
    if (verbose && file_op_compute_totals)
    {
        ...
        else
        {
            ...
            status = compute_dir_size (p, &dsm, &ctx->progress_count, &ctx->progress_bytes,
                                       ctx->follow_links);
            vfs_path_free (p);
        }
...
FileProgressStatus
compute_dir_size (const vfs_path_t * dirname_vpath, dirsize_status_msg_t * sm, size_t * ret_count,
                  uintmax_t * ret_total, gboolean compute_symlinks)
{
    size_t marked = 0;

    return do_compute_dir_size (dirname_vpath, sm, ret_count, &marked, ret_total,
                                compute_symlinks);
}
